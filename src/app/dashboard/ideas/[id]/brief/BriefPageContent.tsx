...
function BackToCampaignLink({ id }: { id: string }) {
  return (
    <Link
...
export function BriefInsufficientDataState({
  id,
  count,
}: {
  id: string;
...
export function BriefFundingGateState({
  id,
  count,
  assumptions,
  briefFundingGate,
      ...
      <div className="rounded-[28px] bg-white border border-border-light shadow-card p-[32px]">
        <div className="text-center mb-6">
          ...
          <p className="text-[14px] text-text-secondary max-w-[440px] mx-auto leading-relaxed">
            ...
            assumption verdicts, evidence synthesis, and next steps, fund your campaign with at least ${briefFundingGate}.
          </p>
        </div>
          ...
          <div className="border-t border-border-light/40 pt-6 mt-6">
            ...
            <ul className="space-y-2">
              {assumptions.map((assumption, index) => (
                <li key={index} className="flex items-start gap-2 text-[14px] text-text-secondary">
                  <span className="font-mono text-[12px] font-bold text-text-muted mt-0.5 shrink-0">
                    {index + 1}.
                  </span>
    ...
    <article className="max-w-[720px] mx-auto px-4 py-12 pb-24">
      ...
      <section className="rounded-[24px] bg-white border border-border-light shadow-card p-[24px] mb-8">
        <div className="flex flex-wrap gap-6 max-md:gap-4">
          ...
          <div>
            ...
            <span className="font-mono text-[20px] font-bold tracking-tight text-text-primary">{brief.assumptionVerdicts.length}</span>
          </div>
          {roundNumber > 1 && (
            <div>
      ...
      <section className="mb-8">
        ...
        <div className="space-y-4">
          {brief.assumptionVerdicts.map((verdict, index) => (
            <div
              key={index}
            ...
            >
                  ...
                  {verdict.confidence && (
                    <span className={`font-mono text-[11px] font-medium uppercase tracking-wide ${
                      verdict.confidence === "HIGH" ? "text-success"
                        : verdict.confidence === "MEDIUM" ? "text-brand"
                        : "text-text-muted"
                    }`}>
                      {verdict.confidence === "HIGH" ? "High" : verdict.confidence === "MEDIUM" ? "Med" : "Low"}
                    </span>
                  )}
                  <span
                    className={`inline-flex items-center rounded-md px-[10px] py-[4px] font-mono text-[11px] font-medium uppercase tracking-wide ${verdictColors[verdict.verdict]}`}
                  >
              ...
              {(verdict.supportingCount > 0 || verdict.contradictingCount > 0) && (
                <div className="flex gap-4 font-mono text-[11px] font-medium text-text-muted uppercase tracking-wide mb-3">
                  <span>{verdict.supportingCount} supporting</span>
                  <span>{verdict.contradictingCount} contradicting</span>
                  <span>{verdict.totalResponses} total</span>
                </div>
              )}

              {coverage[index] && coverage[index].responseCount > 0 && (
                <div className="rounded-[16px] bg-bg-muted/60 border border-border-light/40 px-4 py-3 mb-3">
                  <div className="flex items-center justify-between mb-[8px]">
                    ...
                    <span className={`font-mono text-[11px] font-medium px-[8px] py-[3px] rounded-md uppercase tracking-wide ${
                      coverage[index].strength === "strong"
                        ? "bg-success/10 text-success"
                        : coverage[index].strength === "moderate"
                          ? "bg-brand/10 text-brand"
                          : "bg-[#94A3B8]/10 text-text-muted"
                    }`}>
                      {coverage[index].strength}
                    </span>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5">
                        <div className="h-[3px] flex-1 bg-bg-muted overflow-hidden">
                          <div className="h-full bg-success" style={{ width: `${coverage[index].avgQuality}%` }} />
                        </div>
                        <span className="font-mono text-[12px] font-bold text-text-primary">{coverage[index].avgQuality}</span>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-1.5">
                        <div className="h-[3px] flex-1 bg-bg-muted overflow-hidden">
                          <div className="h-full bg-info" style={{ width: `${coverage[index].avgMatch}%` }} />
                        </div>
                        <span className="font-mono text-[12px] font-bold text-text-primary">{coverage[index].avgMatch}</span>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {coverage[index].categories.map((category) => (
                          <span key={category} className={`font-mono text-[11px] font-medium px-[6px] py-[2px] rounded-md uppercase tracking-wide ${
                            category === "negative" ? "bg-error/10 text-error" : "bg-bg-muted text-text-muted"
                          }`}>
                            {category}
                          </span>
                        ))}
              ...
              {verdict.quotes.length > 0 && (
                <div className="space-y-2 mb-3">
                  {verdict.quotes.map((quote, quoteIndex) => (
                    <blockquote
                      key={quoteIndex}
        ...
        <section className="mb-8">
          ...
          <div className="rounded-[24px] bg-white border border-border-light shadow-card p-[24px]">
            ...
            <div className="space-y-2">
              {brief.assumptionVerdicts.map((verdict) => {
                const previousVerdict = parentVerdicts.verdicts.find(
                  (item) => item.assumption === verdict.assumption
                ) ?? parentVerdicts.verdicts[verdict.assumptionIndex];
                if (!previousVerdict) return null;

                const changed = previousVerdict.verdict !== verdict.verdict;
                const improved = (previousVerdict.verdict === "REFUTED" || previousVerdict.verdict === "CHALLENGED" || previousVerdict.verdict === "INSUFFICIENT_DATA")
                  && (verdict.verdict === "CONFIRMED" || (previousVerdict.verdict === "REFUTED" && verdict.verdict === "CHALLENGED"));
                const regressed = (previousVerdict.verdict === "CONFIRMED" || previousVerdict.verdict === "CHALLENGED")
                  && (verdict.verdict === "REFUTED" || (previousVerdict.verdict === "CONFIRMED" && verdict.verdict === "CHALLENGED"));

                return (
                  <div
                    key={verdict.assumptionIndex}
                  ...
                  >
                    <span className="font-medium block mb-1">{verdict.assumption}</span>
      ...
      <section className="mb-8">
        ...
        <div className="rounded-[24px] bg-white border border-border-light shadow-card p-[24px]">
          <ul className="space-y-2">
            {brief.strongestSignals.map((signal, index) => (
              <li key={index} className="flex items-start gap-2.5 text-[14px] text-text-primary font-medium leading-relaxed">
          ...
          <div className="rounded-[24px] bg-white border border-border-light shadow-card p-[24px]">
            ...
            {Object.keys(priceSignal.priceCeilingDistribution).length > 0 && (
              <div className="mb-4">
                ...
                <div className="space-y-1.5">
                  {Object.entries(priceSignal.priceCeilingDistribution).map(([tier, respondentCount]) => (
                      ...
                      <div className="flex-1">
                        ...
                        <div className="h-1.5 rounded-full bg-border-light">
                          <div
                            ...
                            style={{ width: `${(respondentCount / priceSignal.respondentCount) * 100}%` }}
                          />
                        </div>
                      </div>
                    </div>
            ...
            {Object.keys(priceSignal.pastSpendingDistribution).length > 0 && (
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(priceSignal.pastSpendingDistribution).map(([tier, respondentCount]) => (
                    <span key={tier} className="inline-flex items-center gap-1.5 rounded-full bg-bg-muted px-3 py-1 text-[12px] text-text-secondary">
                      {tier} <span className="font-semibold text-text-primary">{respondentCount}</span>
                    </span>
                  ))}
                </div>
              </div>
            ...
            {priceSignal.forwardWtpDistribution && Object.keys(priceSignal.forwardWtpDistribution).length > 0 && (
              <div className="mb-4">
                ...
                <div className="space-y-1.5">
                  {Object.entries(priceSignal.forwardWtpDistribution).map(([tier, respondentCount]) => (
                    <div key={tier} className="flex items-center gap-3">
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-0.5">
                          <span className="text-[13px] text-text-secondary">{tier}</span>
            ...
            {priceSignal.preferredModelDistribution && Object.keys(priceSignal.preferredModelDistribution).length > 0 && (
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(priceSignal.preferredModelDistribution).map(([model, respondentCount]) => (
                    <span key={model} className="inline-flex items-center gap-1.5 rounded-full bg-bg-muted px-3 py-1 text-[12px] text-text-secondary">
                      {model} <span className="font-semibold text-text-primary">{respondentCount}</span>
                    </span>
                  ))}
                </div>
              </div>
        ...
        <section className="mb-8">
          ...
          <div className="rounded-[24px] bg-white border border-border-light shadow-card p-[24px]">
            ...
            <div className="space-y-3">
              {consistencyReport.gaps.map((gap, index) => (
                <div
                  key={index}
                  className={`rounded-xl px-4 py-3 text-[13px] leading-relaxed ${
                    gap.severity === "high"
      ...
      {segmentReport && segmentReport.disagreements.length > 0 && (
        <section className="mb-8">
          ...
          <div className="rounded-[24px] bg-white border border-border-light shadow-card p-[24px]">
            ...
            <div className="space-y-3">
              {segmentReport.disagreements.map((disagreement, index) => (
                <div
                  key={index}
                  className={`rounded-xl px-4 py-3 text-[13px] leading-relaxed ${
                ...
                >
                  <span className="font-medium">
                    {disagreement.assumption}
                  </span>
                  <p className="mt-1">{disagreement.signal}</p>
                  <div className="flex gap-4 text-[11px] mt-2 opacity-70">
                    <span>High-match: {Math.round(disagreement.highMatchSupportRatio * 100)}% support (n={disagreement.highMatchCount})</span>
                    <span>Low-match: {Math.round(disagreement.lowMatchSupportRatio * 100)}% support (n={disagreement.lowMatchCount})</span>
                  </div>
                </div>
              ))}
            </div>
