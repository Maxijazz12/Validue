...
export default function FeedInterstitial(props: Props) {
    ...
    return (
      <div className="col-span-full">
        <a
          ...
          className="flex items-center gap-[16px] p-[16px] rounded-xl border border-[#E2E8F0] no-underline hover:border-[#CBD5E1] transition-all duration-200 group"
          style={{ borderLeftWidth: 3, borderLeftColor: accent, background: `linear-gradient(90deg, ${accent}06, transparent)` }}
        >
          <div className="flex-1 min-w-0">
            <p className="text-[11px] font-semibold uppercase tracking-[1px] mb-[4px]" style={{ color: accent }}>
              Hot in {props.hotIdea.category}
            </p>
            <p className="text-[14px] font-semibold text-[#111111] truncate group-hover:text-black transition-colors">
              {props.hotIdea.title}
            </p>
          </div>
          {props.hotIdea.rewardAmount > 0 && (
            <span
              className="font-mono font-bold text-[12px] px-[10px] py-[4px] rounded-full text-white shrink-0"
          ...
          <svg
            ...
            className="shrink-0 group-hover:stroke-[#111111] transition-colors"
          >
  ...
  if (props.type === "match-power") {
    return (
      <div className="col-span-full">
        <div className="flex items-center gap-[16px] p-[16px] rounded-xl bg-[#FAF9FA] border border-[#E2E8F0]">
          <div className="w-[40px] h-[40px] rounded-full bg-gradient-to-br from-[#E5654E] to-[#E8C1B0] flex items-center justify-center shrink-0">
          ...
          <div className="flex-1">
            <p className="text-[13px] text-[#64748B]">
              You match <span className="font-bold text-gradient-warm text-[15px]">{props.matchCount}</span> active campaigns
            </p>
            <p className="text-[11px] text-[#94A3B8]">Based on your expertise and interests</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="col-span-full">
        <div className="flex items-center gap-[16px] p-[16px] rounded-xl bg-[#FAF9FA] border border-[#E2E8F0]">
          ...
          <div className="flex-1">
            ...
            <p className="text-[11px] text-[#94A3B8]">Join the conversation</p>
          </div>
        </div>
      </div>
    );
