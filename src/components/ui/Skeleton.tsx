...
export default function Skeleton({ className = "", warm = false }: SkeletonProps) {
  return (
    <div
      className={`${warm ? "skeleton-warm" : "bg-border-light/50 rounded-lg animate-pulse"} ${className}`}
    />
  );
}
...
export function SkeletonCard({ lines = 3 }: { lines?: number }) {
  return (
    <div className="bg-white border border-border-light rounded-2xl p-[20px]">
...
export function SkeletonStatCard() {
  return (
    <div className="bg-white border border-border-light rounded-2xl p-[16px]">
